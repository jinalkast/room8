...
\begin{flushleft}
    ...
    \item \textbf{OUath 2.0:} An open standard protocol for authorization, allowing secure access to user data across different services without revealing login credentials. OAuth is commonly used to enable single sign-on or to grant third-party applications limited access to a user's resources.\newline
...
\begin{flushleft}
  \item \textbf{S.2.1 - User Authentication and House Management}
  	\begin{itemize}
  		...
  		\begin{itemize}
  			\item \textbf{FR211:} The system shall allow users to create an account using their name, email, and password.
  		...
  		\begin{itemize}
  			\item \textbf{NFR2.1.1:} All data related to authentication must be encrypted in both transit and in storage.
  			\item \textbf{NFR2.1.2:} Error messages related to authentication should not disclose sensitive details such as "Incorrect Password".
  			\item \textbf{NFR2.1.3:} All data related to authentication must be encrypted in both transit and in storage.
  			\item \textbf{NFR2.1.4:} The system should be able to authenticate a user with a median response time of under 1 second.
  		\end{itemize}
  	\end{itemize}
  	...
  	\begin{itemize}
  		...
  		\begin{itemize}
  			...
  			\item \textbf{NFR2.4.2:} The calendar system shall display all calendar events to users in their time zone. 
  			\item \textbf{NFR2.4.3:} The calendar system shall have a granularity of 5 minutes.
  		\end{itemize}
  	\end{itemize}
...
\begin{itemize}
    \item Account Creation Flow
    	\begin{itemize}
    		\item  A new user trying to create an account, when sign-up process is started 				they then should be able to enter details and create account with details being stored in secured manner. [NFR2.1.1].
		\end{itemize}    	
    \item Error Handling For Incorrect Information
    	\begin{itemize}
    		\item  A user trying to login to an account, when login process is started 				and they make mistake, (eg. enter wrong password), no information pertaining to sensitive detail should be displayed. [NFR2.1.2].
		\end{itemize}
	\item Account Authentication
    	\begin{itemize}
    		\item  A user trying to login and authenticating their account should be able to have account authenticated must be done in less than 1 second. [NFR2.1.4]. \\
		\end{itemize}		
\textbf{Tests for Calendar Interactions}\\

		\item Calendar Event Creation
    	\begin{itemize}
    		\item  A user who has logged on to the application will create an event then change the time 1 increment above the original time on the calendar and the system will store the event in UTC while the event will show up on the user's device in the user's respective timezone. [NFR2.4.1, NFR2.4.2, NFR2.4.3].
		\end{itemize}
\end{itemize}
