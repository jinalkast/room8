...
export async function PATCH(req: NextRequest): Promise<NextResponse<TApiResponse<TRoommateDB>>> {
  try {
    ...
    if (body.profilePicture) {
      ...
      const { data: img_data, error: img_error } = await supabase.storage
        .from('custom_profile_pictures')
        .update(filePath, blob, { upsert: true, contentType: `image/${imageExtension}` });
      if (img_error) {
        throw img_error;
      }
